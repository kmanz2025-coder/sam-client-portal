...
function renderBudget(){
  ...
  const actualFees=p.invoices.reduce((s,inv)=>s+inv.total,0);
  document.getElementById('budget-kpi').innerHTML=`<div class="bkpi"><div class="bkpi-amount">${fmt(totalBudget||totalPaid)}</div><div class="bkpi-label">Total Budget</div></div><div class="bkpi"><div class="bkpi-amount" style="color:var(--gold)">${fmt(totalPaid)}</div><div class="bkpi-label">Paid to Date</div></div><div class="bkpi"><div class="bkpi-amount" style="color:#6dbf8a">${fmt(actualFees)}</div><div class="bkpi-label">SAM Consulting Fee</div></div>`;
