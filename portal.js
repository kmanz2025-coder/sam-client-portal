...
window._openProject = function(pid){ loadProject(PROJECTS[pid]); window.goTo('dashboard', null); };
...
function renderAdminDashboard(){
  ...
  const projCards = allProjects.map(p=>{
    ...
    return `<div class="invoice-item" style="cursor:pointer;" onclick="window._openProject('${p.id}')">function viewPDF(b64){
  var blob = b64toBlob(b64, 'application/pdf');
  var url = URL.createObjectURL(blob);
  window.open(url, '_blank');
}
...
function renderBudget(){
  ...
  const actualOP=p.invoices.reduce((s,inv)=>s+inv.opAmount,0);
  const actualReimb=p.invoices.reduce((s,inv)=>s+(inv.reimbursable||0),0);
  document.getElementById('budget-kpi').innerHTML=`<div class="bkpi"><div class="bkpi-amount">${fmt(totalBudget||totalPaid)}</div><div class="bkpi-label">Total Budget</div></div><div class="bkpi"><div class="bkpi-amount" style="color:var(--gold)">${fmt(totalPaid)}</div><div class="bkpi-label">Paid to Date</div></div><div class="bkpi"><div class="bkpi-amount" style="color:#6dbf8a">${fmt(actualOP)}</div><div class="bkpi-label">SAM Consulting Fee</div></div>`;
  const invOH=p.invoices.reduce((s,inv)=>s+Math.floor(inv.opAmount/2),0);
  const invPR=p.invoices.reduce((s,inv)=>s+Math.ceil(inv.opAmount/2),0);
  document.getElementById('op-row').innerHTML=`<div class="op-chip">Overhead 4% = ${fmt(invOH)}</div><div class="op-chip">Profit 4% = ${fmt(invPR)}</div>${actualReimb>0?`<div class="op-chip" style="color:var(--muted);">Reimbursables (pass-through): ${fmt(actualReimb)}</div>`:''}<div class="op-chip" style="background:rgba(74,148,100,0.15);border-color:rgba(74,148,100,0.3);color:#6dbf8a;">SAM Fee: ${fmt(actualOP)}</div>`;
